...
%    add_env("identity", [function, [integer], [integer]], A2, A3), % 
%    add_env("identity", [function, [float], [float]], A3, A4).
% HOLY SHIT IM SMART
    add_env("identity", [function, [A], [A]], A2, A3),
    add_env("some-fun", function([type(integer)], [type(float)]), A3, A4),
...
test(11, A) :- type_of([ [application, [id, "map"], [id, "some-fun"], [list, [[int, 1]]]] ], A).
